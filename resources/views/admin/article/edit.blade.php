            ...
            <div class="col-md-10 col-md-offset-1">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form action="{{ url('admin/article/'.$item->id) }}" method="POST">
                            {{ method_field('PUT') }}
                            {!! csrf_field() !!}
                            <input type="hidden" name="id" value="{{$item->id}}">
                            <input type="text" name="title" class="form-control" required="required"
                                   placeholder="请输入系统标题及描述，举个栗子[SRM - 外网]" value="{{$item->title}}">
                            <br>
                            <textarea name="body" rows="5" class="form-control" required="required"
                                      placeholder="请输入从浏览器地址栏复制的完整域名，举个栗子[http://111.207.40.134/]">{{$item->body}}</textarea>
                            <br>
